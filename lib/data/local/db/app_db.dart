...
class AppDb extends _$AppDb {
  ...
  final existingCodigos = (select(viviendaUbicacion)..addColumns([viviendaUbicacion.codigoVivienda])).get();
  ...
  final missingRows = await (select(viviendaUbicacion)..addColumns([viviendaUbicacion.codigoVivienda]))
      .get()
      .then((existingCodigos) => missingCodigos.where((codigo) => !existingCodigos.map((row) => row.codigoVivienda).contains(codigo)));
  ...
  for (final codigo in missingRows) {
    ...
    try {
      ...
      if (e is Exception && e.toString().contains("UNIQUE constraint failed")) {
